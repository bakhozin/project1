	...
	<nav class="navbar navbar-default">
		<div class="container-fluid">
			<div class="navbar-header">
				 ...
				 <a class="navbar-brand" href="../menu/main.do">DD SHOP</a>
			</div>
...
<!-- 			상단 BAR안에 개별적인 링크가 들어간다 -->
				<ul class="nav navbar-nav">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu">
							<li><a href="preview.do">PREVIEW</a></li>
							<li><a href="notice.do">자유게시판</a></li>
							<li><a href="#">고객센터</a></li>
						</ul>
					</li>
			...
			<div class="panel-body">
				<div class="media">
					<div class="media-left">
						<a href="https://www.musinsa.com/?m=news"><img class="media-object" src="${p.image}" alt="차범근"></a>
					</div>
					<div class="media-body">
					<c:if test="${p.seq>4 || p.seq==2}">
						<h4 class="media-heading"><a href="https://www.musinsa.com/?m=news">${p.title}</a></h4>						
					</c:if>
					<c:if test="${p.seq==1 || p.seq==3 || p.seq==4}">
						<h4 class="media-heading"><a href="https://www.musinsa.com/?m=news">${p.title}</a>&nbsp;<span class="badge">New</span></h4>
					</c:if>
						<a href="https://www.musinsa.com/?m=news">${p.content}</a>
						<div style="float: right"><br><br><br><br>${p.regdate}</div>
					</div>
				</div>
				<hr>
			</div>
						...
						<c:if test="${sPage!=1}">
							<p id="btnPrev">
								<a class="button btn-prev"
									href="preview.do?pg=${sPage-1}&f=${field}&q=${urlQuery}">다음</a>
							</p>
						</c:if>
							...
							<c:forEach var="i" begin="0" end="4">
								<li>
								    <c:if test="${i+sPage <= finalPage}">								
											<a href="preview.do?pg=${sPage+i}&f=${field}&q=${urlQuery}">${sPage+i}</a>
									</c:if>
								</li>
							</c:forEach>
						</ul>
						<c:if test="${sPage+4 < finalPage}">
							<p id="btnNext">
								<a class="button btn-next"
									href="preview.do?pg=${sPage+5}&f=${field}&q=${urlQuery}">다음</a>
							</p>
						</c:if>
					</div>
				</div>
